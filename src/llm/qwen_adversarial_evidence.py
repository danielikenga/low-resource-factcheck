...
RESULTS_PATH = "results/llm/qwen_adversarial_evidence_results.json"
PREDICTIONS_PATH = "results/llm/qwen_adversarial_evidence_predictions.csv"
...
def create_adversarial_evidence(df, current_row):
    candidates = df[
        (df["label"] != current_row["label"]) &
        (df["language"] == current_row["language"]) &
        (df["extracted_evidence_text"].notna())
    ]
    ...
    sampled_row = candidates.sample(
        n=1,
        random_state=int(abs(hash(current_row["id"])) % (10**8))
    ).iloc[0]

    return sampled_row["extracted_evidence_text"], sampled_row["id"], sampled_row["label"]
...
def main():
    ...
    for idx, row in test_df.iterrows():
        ...
        evidence, adversarial_id, adversarial_label = create_adversarial_evidence(df, row)
        ...
        predictions.append({
            ...
            "adversarial_source_id": adversarial_id,
            "adversarial_source_label": adversarial_label,
